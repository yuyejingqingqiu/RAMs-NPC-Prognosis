...
required_cols <- c("PFS_time", "PFS_status", "T_stage", "N_stage", "LDH", "NLR", "GTVnx", "RAMs_score", "Rad_score", "Path_score")
...
fit_clinical <- cph(Surv(PFS_time, PFS_status) ~ T_stage + N_stage + LDH + NLR + GTVnx, data = df, x = TRUE, y = TRUE, surv = TRUE)
fit_RAMs <- cph(Surv(PFS_time, PFS_status) ~ RAMs_score, data = df, x = TRUE, y = TRUE, surv = TRUE)
fit_nomo <- cph(Surv(PFS_time, PFS_status) ~ RAMs_score + T_stage + N_stage + LDH + NLR + GTVnx, data = df, x = TRUE, y = TRUE, surv = TRUE)
...
roc_rams_36 <- timeROC(T = df$PFS_time, delta = df$PFS_status, marker = df$RAMs_score, cause = 1, weighting = "marginal", times = 36, iid = TRUE)
cat(sprintf("3-year time-dependent AUC for RAMs: %.3f\n", roc_rams_36$AUC[2]))
 
df$event_36 <- ifelse(df$PFS_status == 1 & df$PFS_time <= 36, 1, ifelse(df$PFS_time >= 36, 0, NA))
...
idi_nri_results <- IDI.INF(indata = as.matrix(df[, c("PFS_time", "PFS_status")]), covs0 = covs_clinical, covs1 = covs_RAMs, t0 = 36, npert = 300)
print(idi_nri_results)
 
cal_RAMs <- calibrate(fit_RAMs, cmethod = "KM", method = "boot", u = 36, m = 5, B = 200)
pdf("Figure_Calibration_RAMs.pdf", width = 6, height = 6)
plot(cal_RAMs, lwd = 2, xlab = "Predicted 3-year PFS probability", ylab = "Observed 3-year PFS probability")
dev.off()
 
df$pred_clinical <- 1 - survest(fit_clinical, newdata = df, times = 36)$surv
df$pred_RAMs <- 1 - survest(fit_RAMs, newdata = df, times = 36)$surv
dca_results <- dca(Surv(PFS_time, PFS_status) ~ pred_clinical + pred_RAMs, data = df, time = 36, thresholds = seq(0.01, 0.85, by = 0.01))
...
nomogram_obj <- nomogram(fit_nomo, fun = list(surv_1yr, surv_3yr), lp = TRUE, funlabel = c("1-year PFS probability", "3-year PFS probability"))
