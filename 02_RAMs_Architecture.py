...
class RAMsFusionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.attn_level1 = nn.MultiheadAttention(
            hidden_dim, num_heads, dropout=dropout, batch_first=True
        )
        self.attn_level2 = nn.MultiheadAttention(
            hidden_dim, num_heads, dropout=dropout, batch_first=True
        )
    ...
    def forward(self, radiomics, clinical, pathomics):
        ...
        clin_rad_repr, _ = self.attn_level1(
            query=q_rad,
            key=kv_clin,
            value=kv_clin,
        )
        ...
        path_recalibrated, _ = self.attn_level2(
            query=joint_repr,
            key=kv_path,
            value=kv_path,
        )
